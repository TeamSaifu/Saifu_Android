...
class Shortcut4Widget : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            BUTTON1 -> shortcut(context, intent)
            BUTTON2 -> shortcut(context, intent)
            BUTTON3 -> shortcut(context, intent)
            BUTTON4 -> shortcut(context, intent)
        }
    }
    ...
    private fun shortcut(context: Context, intent: Intent) {
        ...
        Log.e("widget", "$name : $price : $category")
        if (sharedPref.getBoolean("shortCutQuickAdd", false)) {
            ...
            val dataIntent = Intent(context, DataInputActivity::class.java)
            dataIntent.putExtra("mode", "Shortcut")
            dataIntent.putExtra("price", price)
            dataIntent.putExtra("name", name)
            dataIntent.putExtra("category", category)
            startActivity(context, dataIntent, null)
        }
    }
